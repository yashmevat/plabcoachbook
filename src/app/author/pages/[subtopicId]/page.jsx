...
useEffect(() => {
 ...
 imageResize: {
  ...
  const updatePageStatus = (quill, container) => {
    ...
    // Measure rendered content height first
    let contentHeight = 0;
    try {
    ...
    // Fallback to word-count if measurement fails
 ...
 imageResize: {
    modules: ['Resize', 'DisplaySize', 'Toolbar']
      ...
      // Snapshot safe HTML to revert if user overflows the A4 page while editing
      quill.__lastSafeHtml = quill.root.innerHTML;

      quill.on('text-change', (delta, oldDelta, source) => {
        const content = quill.root.innerHTML;

        // If change came from user and now exceeds page height, revert to last safe state
        if (source === 'user') {
          let contentHeight = 0;
          try {
            const temp = document.createElement('div');
        ...
        // Accept change and update snapshot
        setLivePages(prev => {
          const updated = [...prev];
